...
module Proxy::OpenSCAP
  ...
  class Api < ::Sinatra::Base
    ...
    post "/oval_report/:oval_policy_id" do
      oval_policy_id = params[:oval_policy_id]
      begin
        Proxy::OpenSCAP::OvalReportStorageFs.new(
          Proxy::OpenSCAP::Plugin.settings.reportsdir,
          oval_policy_id,
          @cn,
          @reported_at
        ).store_report(request.body.string)

        { :result => :ok, :reported_at => @reported_at }.to_json
      rescue Proxy::OpenSCAP::StoreReportError => e
        logger.error e
        { :result => 'Storage failure on proxy, see proxy logs for details.' }.to_json
      rescue Proxy::OpenSCAP::OpenSCAPException => e
    ...
    get "/oval_policies/:oval_policy_id/content/:digest" do
      content_type 'application/xml'
      begin
        Proxy::OpenSCAP::FetchScapFile.new(:oval_content)
          .fetch(params[:policy_id], params[:digest], Proxy::OpenSCAP::Plugin.settings.oval_content_dir)
      rescue *HTTP => e
        log_halt e.response.code.to_i, file_not_found_msg
      rescue StandardError => e
        log_halt 500, "Error occurred: #{e.message}"
